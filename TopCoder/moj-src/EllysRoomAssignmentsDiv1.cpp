...
double EllysRoomAssignmentsDiv1::getAverage(vector <string> ratings) {
	string s= "";
	rep(i, 0, sz(ratings)) s+=ratings[i];
	istringstream sin(s);
	vector<int> v;
	int a;
	...
	vector<VI> vv;
	for(int i=0;i<sz(v);i+=R) {
		VI vi;
		for(int j=0;j<R;j++) {
			if(i+j < sz(v)) vi.push_back(v[i+j]);
		}
		bool have = false;
		for(int j=0;j<sz(vi);j++) if(vi[j]==my) have = true;
		if(have==false) vv.push_back(vi);
	}
	...
	for(int i=0;i<sz(vv);i++) {
		//rep(j, 0, sz(vv[i])) cout<<vv[i][j]<<' '; cout<<endl;
		if(sz(vv[i]) == R) {
			double t = 0.0;
			rep(j, 0, sz(vv[i])) t += vv[i][j];
			t/=R;
			ret += t;
			num++;
		} else {
	...
	double ave = 0.0;
	rep(i, 0, sz(tmp)) ave += tmp[i];
	ave /= sz(tmp);
	//printf("num = %d\n", num);
	//printf("ret = %lf, ave = %lf\n", ret, ave);

	return (ret+ave)/(num+1)*sz(tmp)/R + ret/num * (R-sz(tmp))/(double)R;
}
