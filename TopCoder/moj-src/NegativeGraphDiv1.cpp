...
struct Matrix{
	...
	Matrix(int r=0,int c=0):R(r),C(c){ memset(d, 0x1f,sizeof(d)); }
...
long long NegativeGraphDiv1::findMin(int N, vector <int> from, vector <int> to, vector <int> weight, int charges) {
	clr(dis, 0x3f);
	clr(ma, 0);
	rep(i, 1, N+1) dis[i][i] = 0;
	rep(i, 0, sz(from)) {
		dis[from[i]][to[i]] = min(dis[from[i]][to[i]], weight[i]);
		ma[from[i]][to[i]] = max(ma[from[i]][to[i]], weight[i]);
	}
	rep(k, 1, N+1) rep(i, 1, N+1) rep(j, 1, N+1) dis[i][j] = min(dis[i][j], dis[i][k] + dis[k][j]);

	Matrix m(N, N);
	//rep(i, 1, N+1) rep(j, 1, N+1) m[i-1][j-1] = 100000 * 555;
	rep(i, 1, N+1) rep(j, 1, N+1) rep(k, 0, sz(from)) if(from[k]==j) {
		m[i-1][to[k]-1] = min(m[i-1][to[k]-1], (LL)dis[i][j] - weight[k]);
	}

	LL ret = dis[1][N];

	for(int c=1;c<=min(55, charges);c++) {
		Matrix tmp = m^c;
		rep(j, 1, N+1) ret = min(ret, tmp[0][j-1] + dis[j][N]);
	}

	Matrix tmp = m^charges;
	rep(j, 1, N+1) ret = min(ret, tmp[0][j-1] + dis[j][N]);
