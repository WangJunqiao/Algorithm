...
int AntsMeet::countAnts(vector <int> x, vector <int> y, string D) {
	vector<PII> dirs;
	repv(i, x) {
		x[i]*=2;
		y[i]*=2;
	...
	int n=sz(x);
	rep(i, 0, n){
		if( D[i]=='E' ) dirs.PB(MP(1, 0));
		else if( D[i]=='W' ) dirs.PB(MP(-1, 0));
		else if( D[i]=='S' ) dirs.PB(MP(0, -1));
		else dirs.PB(MP(0, 1));
	}
	
	bool vst[55]={false};
	rep(ti, 0, 5000){
		repv(i, x) x[i]+=dirs[i].ff;
		repv(i, y) y[i]+=dirs[i].ss;
		repv(i, x) if( vst[i]==false ){
			int cnt=0;
			repv(j, x) if( j>=i && vst[j]==false && x[i]==x[j] && y[i]==y[j] )
				cnt++;
			if( cnt>=2 ){
				repv(j, x) if( j>=i && vst[j]==false && x[i]==x[j] && y[i]==y[j]) {
					vst[j]=true;
