...
int dis[33][33], n, ans=0;

int simu(int cur, int msk){
	int mi=1000000000, id=-1;
	rep(i, 0, n) if( test(msk, i)==0 ){
		if( dis[cur][i]<mi ){
			mi=dis[cur][i];
			id=i;
		}
	...
	if( id==-1 ) return 0;
	return dis[cur][id]+simu(id, msk|(1<<id));
}

void dfs(int cur, int msk, int cost){
	int mi=1000000000, id=-1;
	int num=0;
	rep(i, 0, n) if( test(msk, i)==0 ){
		num++;
	...
	rep(i, 0, n) if( test(msk, i) ==0 ){
		if( i==id ){
			ans=max(ans, cost+dis[cur][i]+simu(i, msk|(1<<i)) );
		}else{
			ans=max(ans, cost+dis[cur][id]-1+simu(i, msk|(1<<i)) );
		}
	}
	dfs(id, msk|(1<<id), cost+dis[cur][id]);
}

int GreedyTravelingSalesman::worstDistance(vector <string> qian, vector <string> bai, vector <string> shi, vector <string> ge) {
	n=sz(qian);
	rep(i, 0, n) rep(j, 0, n) dis[i][j]=1000*(qian[i][j]-'0')+100*(bai[i][j]-'0') + 10*(shi[i][j]-'0')+(ge[i][j]-'0');
	ans=0;
	dfs(0, 1, 0);
	return ans;
}
