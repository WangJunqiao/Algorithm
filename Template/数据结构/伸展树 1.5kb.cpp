...
struct Node{
	...
	int64 add; //延迟标记
	//上面数据
	Node* ch[2], *f; int size;
	void init(){ 
		f=ch[0]=ch[1]=NULL; size=1;  //必须
		add=0; //延迟标记清0
	}
	void rot(int c){//c=0左旋,c=1右旋
		...
		f = z ; if( z ) if( y==z->ch[0]) z->ch[0]=this; else z->ch[1]=this;
		ch[c]=y; y->f=this;
		y->update();//x节点最后才维护的
	}
	void splay(Node *fa){//将自己旋转到father下面
		for(push_down(); f != fa; ){
			if( f->f == fa) {
				...
				Node *y=f , *z=y->f;
				if( y->ch[0] == this){
					if( z->ch[0]==y ) { y->rot(1); rot(1); }
					else{ rot(1); rot(0); }
				}else{
					if( z->ch[0]!=y ) { y->rot(0); rot(0); }
					else{ rot(0); rot(1); }
				}
			}
		}
	...
	void push_down(){//将延迟信息push下去
		if( add ){
			if( ch[0] ) {
				ch[0]->add+=add; ch[0]->val+=add; ch[0]->sum+=ch[0]->size * add;
			}
			if( ch[1] ) {
				ch[1]->add+=add; ch[1]->val+=add; ch[1]->sum+=ch[1]->size * add;
			}
			add=0;
		}
	}
	...
	while( scanf("%d %d",&n,&m)!=EOF ){ //序列长度，操作次数
		...
		for(int i=1;i<=m;i++){
			...
			if( op=='a' ){
				...
				tmp->splay(root);
				tmp=tmp->ch[0];
				tmp->val+=val; tmp->add+=val; tmp->sum+=tmp->size * (int64)val;
				tmp->splay(NULL); root=tmp;
			}else{
				scanf("%d %d",&a,&b); a++,b++;
				tmp=root->find(a-1);
