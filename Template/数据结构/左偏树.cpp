...
struct Node{
	type key;
	//以上
	int dis;  Node *ls,*rs,*pa;
	void init(type _key){ key=_key; dis=0; ls=rs=pa=NULL;  }
}dd[maxn];
int e;  //每次初始化为0
struct LeftistTree{
	Node *root;  int size;
	...
	void pop(){ //删除堆首元素
		size--; root=_Merge(root->ls,root->rs);
	}
	type& top()const{  return root->key; } //取堆首元素
	void putAll(LeftistTree &L){ //插入整棵树
		size+=L.size; root=_Merge(root,L.root);
	}
	...
	Node* _Merge(Node* A,Node* B){
		...
		A->rs=_Merge(A->rs,B);  A->rs->pa=A;
		if( _d(A->ls) < _d(A->rs) ) swap(A->ls,A->rs);
		A->dis=_d(A->rs)+1;   return A;
	}
}st[maxn];//*************模板结束****************
