...
void access(Node *p){//s是p下面一条路径的根节点
	...
	while( p!=NULL ){
		...
		if( p->ch[1] ) { //安顿后继
			...
			p->ch[1]->up=p; p->ch[1]->f=NULL; //f=NULL表示将它设置为树根
		} //p是根
		p->ch[1]=s;  
		if( s ) { s->up=NULL; s->f=p; s->splay(NULL); }//连接  s旋转到根,保持各种更新，延迟信息
		else{  s=p; s->update(); } //s是根了
		p=s->find(1)->up;
	}
}
